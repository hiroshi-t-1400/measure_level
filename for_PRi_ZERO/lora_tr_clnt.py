...
def measure_distance(times): # times の引数に測定する回数を受け取る
    
    # trig_pin = 26
    # echo_pin = 16
    # max_distance=6.5m
    sensor = DistanceSensor(echo=16, trigger=26, max_distance=6.5)

    distance = []

    # while True:
    for loop in range(times): # for in range(n) でn回ループする
        distance.append(int(sensor.distance * 1000)) # 測定値をリストに詰めていく
        time.sleep(1)

    distance.sort() # リストをソート**タプルは不可
    length_distance = len(distance)

    get_data = distance[3:length_distance-3] # 最大最小３つずつ捨てる
...
while True:
    ...
    #current_time = int(time.time()) # 4バイト
    current_time = int(time.time()) # 4バイト
    print(f"{tm} :測定開始")
    get_data = measure_distance(10)
    print("測定終了")
    print(get_data)

    txdt = (current_time, *tuple(get_data))

    lora_transmission(uart, txdt)
